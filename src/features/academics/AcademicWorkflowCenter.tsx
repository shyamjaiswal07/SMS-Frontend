...
export default function AcademicWorkflowCenter() {
  ...
  const [selectedAssignmentId, setSelectedAssignmentId] = useState<
    number | null
  >(null);
  const [uploadedSubmissionAssetId, setUploadedSubmissionAssetId] = useState<
    number | null
  >(null);
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setSubmissions(rowsOf(submissionData) as Submission[]);
      ...
      message.error(
        parseApiError(error, "Failed to load academic workflow workspace"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const studentMap = useMemo(
    () =>
      new Map(
        students.map((item) => [
          item.id,
          `${item.student_id ?? item.id} - ${item.first_name ?? ""} ${item.last_name ?? ""}`.trim(),
        ]),
      ),
    [students],
  );
  const termMap = useMemo(
    () =>
      new Map(terms.map((item) => [item.id, item.name ?? `Term #${item.id}`])),
    [terms],
  );
  const sectionMap = useMemo(
    () =>
      new Map(
        sections.map((item) => [item.id, item.name ?? `Section #${item.id}`]),
      ),
    [sections],
  );
  const courseMap = useMemo(
    () =>
      new Map(
        courses.map((item) => [
          item.id,
          `${item.code ?? item.id} - ${item.title ?? ""}`.trim(),
        ]),
      ),
    [courses],
  );
  ...
  const visibleSubmissions = useMemo(
    () =>
      selectedAssignmentId
        ? submissions.filter((item) => item.assignment === selectedAssignmentId)
        : submissions,
  ...
  const calendarColumns: ColumnsType<CalendarEvent> = [
    ...
    },
    {
      ...
      render: (value) => (
        <span className="text-white/70">
          {value ? (courseMap.get(value) ?? `#${value}`) : "-"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      dataIndex: "starts_at",
      render: (value) => (
        <span className="text-white/55">{formatDateTime(value)}</span>
      ),
    },
    {
      title: "Ends",
  ...
  const assignmentColumns: ColumnsType<Assignment> = [
    ...
    },
    {
      ...
      dataIndex: "status",
      render: (value) => (
        <Tag color={assignmentStatusColor(value)}>{value}</Tag>
      ),
    },
    {
      title: "Actions",
        ...
        <Space wrap>
          <Button
            ...
            onClick={() => {
              ...
              submissionForm.setFieldValue("assignment_id", row.id);
          ...
          {row.status === "DRAFT" ? (
            <Button
              size="small"
              loading={actionKey === `publish-${row.id}`}
              onClick={async () => {
                setActionKey(`publish-${row.id}`);
                try {
                  await academicOperationsApi.workflow.publishAssignment(
                    row.id,
                  );
                  ...
                  message.error(
                    parseApiError(error, "Unable to publish assignment"),
                  );
                } finally {
                  setActionKey(null);
                }
          ...
          {row.status === "PUBLISHED" ? (
            <>
              <Button
                ...
                onClick={() => {
                  ...
                  extendForm.setFieldsValue({
                    ...
                    late_due_at: row.late_due_at ?? undefined,
                  });
                }}
              >
                Extend
              ...
              <Button
                size="small"
                loading={actionKey === `close-${row.id}`}
                onClick={async () => {
                  setActionKey(`close-${row.id}`);
                  try {
                    await academicOperationsApi.workflow.closeAssignment(
                      row.id,
                    );
                    ...
                    message.error(
                      parseApiError(error, "Unable to close assignment"),
                    );
                  } finally {
                    setActionKey(null);
                  }
          ...
          {row.status !== "ARCHIVED" && row.status !== "CANCELLED" ? (
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  await academicOperationsApi.workflow.cancelAssignment(row.id);
                  message.success("Assignment cancelled");
                  await loadAll();
                } catch (error) {
                  message.error(
                    parseApiError(error, "Unable to cancel assignment"),
                  );
                } finally {
                  setActionKey(null);
                }
          ...
          {row.status === "CLOSED" || row.status === "CANCELLED" ? (
            <Button
              size="small"
              loading={actionKey === `archive-${row.id}`}
              onClick={async () => {
                setActionKey(`archive-${row.id}`);
                try {
                  await academicOperationsApi.workflow.archiveAssignment(
                    row.id,
                  );
                  ...
                  message.error(
                    parseApiError(error, "Unable to archive assignment"),
                  );
                } finally {
                  setActionKey(null);
                }
  ...
  const submissionColumns: ColumnsType<Submission> = [
    {
      ...
      render: (value) => (
        <span className="text-white/80">
          {value ? (studentMap.get(value) ?? `#${value}`) : "-"}
        </span>
      ),
    },
    {
      ...
      dataIndex: "status",
      render: (value) => (
        <Tag color={submissionStatusColor(value)}>{value}</Tag>
      ),
    },
    {
      title: "Marks",
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <Typography.Paragraph className="!mb-0 !text-white/60">
            Sprint 9 academic calendar, assignment lifecycle, student
            submissions, and grading workflows.
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}>
          <Card className="!bg-[var(--cv-card)] !border-white/10 !rounded-3xl">
            <Statistic
              title={<span className="text-white/60">Calendar Events</span>}
              value={events.length}
        ...
        <Col xs={24} md={8}>
          <Card className="!bg-[var(--cv-card)] !border-white/10 !rounded-3xl">
            <Statistic
              title={<span className="text-white/60">Assignments</span>}
              value={assignments.length}
              prefix={<FileTextOutlined className="text-[var(--cv-accent)]" />}
              valueStyle={{ color: "#e5e7eb" }}
            />
          </Card>
        </Col>
        <Col xs={24} md={8}>
          <Card className="!bg-[var(--cv-card)] !border-white/10 !rounded-3xl">
            <Statistic
              title={<span className="text-white/60">Submissions</span>}
              value={submissions.length}
              prefix={
                <CheckCircleOutlined className="text-[var(--cv-accent)]" />
              }
              valueStyle={{ color: "#e5e7eb" }}
            />
          </Card>
                ...
                <Card className="!bg-[var(--cv-card)] !border-white/10 !rounded-3xl">
                  ...
                  >
                        ...
                        <Form.Item name="term" label="Term">
                          <Select
                            ...
                            options={terms.map((item) => ({
                              value: item.id,
                              label: termMap.get(item.id) ?? item.name,
                            }))}
                          />
                        </Form.Item>
                      </Col>
                        ...
                        <Form.Item name="section" label="Section">
                          <Select
                            ...
                            options={sections.map((item) => ({
                              value: item.id,
                              label: sectionMap.get(item.id) ?? item.name,
                            }))}
                          />
                        </Form.Item>
                      </Col>
                    ...
                    <Form.Item name="course" label="Course">
                      <Select
                        ...
                        options={courses.map((item) => ({
                          value: item.id,
                          label: courseMap.get(item.id) ?? item.title,
                        }))}
                      />
                    </Form.Item>
                    <Row gutter={12}>
                    ...
                    <Row gutter={12}>
                      ...
                      <Col span={12}>
                        <Form.Item name="color_hex" label="Color">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setSubmitting(true);
                          await academicOperationsApi.workflow.createCalendarEvent(
                            values,
                          );
                ...
                <Card className="!bg-[var(--cv-card)] !border-white/10 !rounded-3xl">
                  ...
                  >
                    ...
                    <Row gutter={12}>
                      ...
                      <Col span={12}>
                        <Form.Item name="late_due_at" label="Late Due At">
                          <Input type="datetime-local" />
                        </Form.Item>
                      </Col>
                    </Row>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setSubmitting(true);
                          await academicOperationsApi.workflow.createAssignment(
                            {
                              ...values,
                              late_due_at: values.allow_late_submission
                                ? values.late_due_at || null
                                : null,
                            },
                          );
                          ...
                          message.error(
                            parseApiError(error, "Unable to create assignment"),
                          );
                        } finally {
                          setSubmitting(false);
                        }
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
                      <div>
                        ...
                        <div className="text-white/55 text-sm">
                          Supports student text submissions and optional file
                          assets.
                        </div>
                      </div>
                      {selectedAssignment ? (
                    ...
                    >
                      ...
                      >
                        <Select
                          ...
                          options={assignments.map((item) => ({
                            value: item.id,
                            label: `${item.title ?? `Assignment #${item.id}`} (${item.status ?? "DRAFT"})`,
                          }))}
                      ...
                      <Form.Item name="student_id" label="Student">
                        <Select
                          ...
                          options={students.map((item) => ({
                            value: item.id,
                            label: studentMap.get(item.id) ?? item.student_id,
                          }))}
                        />
                      </Form.Item>
                      ...
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4 mb-4">
                        ...
                        <FileAssetUploader
                          ...
                          helperText={
                            uploadedSubmissionAssetId
                              ? `Attached asset #${uploadedSubmissionAssetId}`
                              : "Upload a PDF, document, or image to attach to the submission."
                          }
                          onUploaded={(asset) =>
                            setUploadedSubmissionAssetId(asset.id)
                          }
                        />
                      </div>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            if (uploadedSubmissionAssetId)
                              payload.file_asset_id = uploadedSubmissionAssetId;
                            await academicOperationsApi.workflow.submitAssignment(
                  ...
                  <Card className="!bg-[var(--cv-card)] !border-white/10 !rounded-3xl">
                    <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                      <div>
                        ...
                        <div className="text-white/55 text-sm">
                          Review submissions, late work, grades, and
                          resubmissions.
                        </div>
                      </div>
                      <Select
                        ...
                        onChange={(value) =>
                          setSelectedAssignmentId(value ?? null)
                        }
                        options={assignments.map((item) => ({
                          value: item.id,
                          label: item.title ?? `Assignment #${item.id}`,
                        }))}
                      />
                    </div>
                    {selectedAssignment ? (
                        ...
                        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                          ...
                          <div className="text-white font-medium mt-1">
                            {selectedAssignment.allow_late_submission
                              ? formatDateTime(selectedAssignment.late_due_at)
                              : "Disabled"}
                          </div>
                        </div>
                      </div>
      ...
      <Modal
        title={
          extendTarget
            ? `Extend Deadline: ${extendTarget.title ?? `Assignment #${extendTarget.id}`}`
            : "Extend Deadline"
        }
        open={!!extendTarget}
        ...
        onOk={() => {
          void extendForm.validateFields().then(async (values) => {
            ...
            try {
              ...
              if (typeof values.allow_late_submission === "boolean")
                payload.allow_late_submission = values.allow_late_submission;
              if ("late_due_at" in values)
                payload.late_due_at = values.allow_late_submission
                  ? values.late_due_at || null
                  : null;
              await academicOperationsApi.workflow.extendAssignment(
                extendTarget.id,
                payload,
              );
              ...
              message.error(
                parseApiError(error, "Unable to extend assignment deadline"),
              );
            } finally {
              setSubmitting(false);
            }
      ...
      >
        <Form<ExtendFormValues>
          form={extendForm}
          layout="vertical"
          requiredMark={false}
        >
          <Form.Item name="due_at" label="New Due At">
            <Input type="datetime-local" />
          </Form.Item>
          <Form.Item
            name="allow_late_submission"
      ...
      <Modal
        title={
          gradeTarget
            ? `Grade Submission #${gradeTarget.id}`
            : "Grade Submission"
        }
        open={!!gradeTarget}
        ...
        onOk={() => {
          void gradeForm.validateFields().then(async (values) => {
            ...
            setSubmitting(true);
            try {
              await academicOperationsApi.workflow.gradeSubmission(
                gradeTarget.id,
                values,
              );
        ...
        >
          ...
          {gradeTarget ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/60">
              Assignment:{" "}
              <span className="text-white/80">
                {assignments.find((item) => item.id === gradeTarget.assignment)
                  ?.title ?? `#${gradeTarget.assignment}`}
              </span>
              <br />
              Student:{" "}
              <span className="text-white/80">
                {gradeTarget.student
                  ? (studentMap.get(gradeTarget.student) ??
                    `#${gradeTarget.student}`)
                  : "-"}
              </span>
              <br />
              Submitted:{" "}
              <span className="text-white/80">
                {formatDateTime(gradeTarget.submitted_at)}
              </span>
            </div>
          ) : null}
        </Form>
